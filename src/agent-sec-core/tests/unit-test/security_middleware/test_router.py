...
from security_middleware import router


class TestModuleToClassName(unittest.TestCase):
    def test_sandbox(self):
        name = router._module_to_class_name("security_middleware.backends.sandbox")
        self.assertEqual(name, "SandboxBackend")

    def test_asset_verify(self):
        name = router._module_to_class_name("security_middleware.backends.asset_verify")
        self.assertEqual(name, "AssetVerifyBackend")

    def test_hardening(self):
        name = router._module_to_class_name("security_middleware.backends.hardening")
...
class TestRegisterAction(unittest.TestCase):
    ...
    def test_register_new_action(self):
        router.register_action("custom_test", "security_middleware.backends.sandbox")
    ...
    def test_register_invalidates_cache(self):
        ...
        router.register_action("sandbox_prehook", "security_middleware.backends.sandbox")
        b2 = router.get_backend("sandbox_prehook")
