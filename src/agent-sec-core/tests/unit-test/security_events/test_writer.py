...
class TestWriterAutoRotation(unittest.TestCase):
    ...
    def test_auto_rotation_on_size_limit(self):
        ...
        # Original file should exist and be smaller than max_bytes
        self.assertTrue(os.path.exists(self.tmp.name))
        current_size = os.path.getsize(self.tmp.name)
        self.assertLess(current_size, 500)
    ...
    def test_backup_count_limit(self):
        ...
        self.assertLessEqual(
            len(backup_files),
            3,
            f"Should have at most 3 backup files, but found {len(backup_files)}: {backup_files}"
        )
    ...
    def test_oldest_backups_are_deleted(self):
        ...
        # Should have exactly backup_count (3) backup files
        self.assertEqual(
            len(backup_files),
            3,
            f"Should have exactly 3 backup files after cleanup, but found {len(backup_files)}: {sorted(backup_files)}"
        )
        ...
        # Verify current file exists and is smaller than max_bytes
        self.assertTrue(os.path.exists(self.tmp.name))
        current_size = os.path.getsize(self.tmp.name)
        self.assertLess(current_size, 300)
    ...
    def test_cleanup_preserves_most_recent_backups(self):
        ...
        self.assertEqual(
            len(backup_files),
            2,
            f"Should have exactly 2 backup files, but found {len(backup_files)}"
        )
    ...
    def test_cleanup_detailed_verification(self):
        ...
        # Verification 1: Exactly backup_count backups
        self.assertEqual(
            len(backup_files),
            3,
            f"Should have exactly 3 backup files, but found {len(backup_files)}: {backup_files}"
        )
