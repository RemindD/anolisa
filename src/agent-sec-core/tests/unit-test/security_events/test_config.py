...
from security_events.config import FALLBACK_LOG_PATH, PRIMARY_LOG_PATH, get_log_path


class TestGetLogPath(unittest.TestCase):
    @patch("security_events.config.os.access", return_value=True)
    @patch("security_events.config.os.path.isdir", return_value=True)
    @patch("security_events.config.os.makedirs")
    def test_primary_path_when_writable(self, mock_makedirs, mock_isdir, mock_access):
        path = get_log_path()
        self.assertEqual(path, PRIMARY_LOG_PATH)

    @patch("security_events.config.os.access", return_value=False)
    @patch("security_events.config.os.path.isdir", return_value=True)
    @patch("security_events.config.os.makedirs")
