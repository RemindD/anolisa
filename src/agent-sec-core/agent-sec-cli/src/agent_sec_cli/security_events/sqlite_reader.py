...
class SqliteEventReader:
    ...
    def query_last_hours(
        ...
        Designed for SummaryBackend aggregation — no result limit.
        ...
        try:
            conn = self._connect()
            try:
                conditions: list[str] = ["timestamp_epoch >= ?"]
                ...
                sql = (
                    ...
                    f"FROM security_events{where} "
                    "ORDER BY timestamp_epoch DESC "
                    "LIMIT 10000"  # TODO: remove hardcoded limit
                )
                cursor = conn.execute(sql, params)
                rows = cursor.fetchall()
            finally:
    ...
    def count_by(
        ...
        """
        allowed = {"category", "event_type", "trace_id"}
