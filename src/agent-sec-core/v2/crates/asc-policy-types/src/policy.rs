...
/// Immutable reference to the single Effective Policy Snapshot registered with `AgentSight`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct EffectivePolicyRef {
    /// Stable effective-policy identity.
    pub policy_id: PolicyId,
    /// Immutable effective-policy revision.
    pub revision: Revision,
    /// Immutable semantic profile.
    pub profile_id: ProfileId,
    /// Optional canonical payload digest.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub payload_digest: Option<Digest>,
}

/// Immutable policy revision exchanged by PCP and `AgentSight`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
