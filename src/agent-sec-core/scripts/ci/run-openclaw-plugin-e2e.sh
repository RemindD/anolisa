...
# OPENCLAW_VERSION is accepted as the CI input, but deploy.sh also treats that
# name as a host-version override. Capture it, then unset it before child
# processes run so deploy.sh detects the actual installed OpenClaw binary.
